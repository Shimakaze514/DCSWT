...
function SLOT.callbacks.onPlayerTryChangeSlot(playerID, side, slotID)
	...
	if sideAvail == true and balance == true and slotAvail == true then
		...
		net.log(
			'[SLOTAUTH] 玩家 ' ..
				tostring(_playerInfo.name or playerID) ..
					' 成功切换至阵营 ' ..
						tostring(side) ..
							'. SideAvail:' ..
								tostring(sideAvail) .. ', Balance:' .. tostring(balance) .. ', SlotAvail:' .. tostring(slotAvail)
		)
		return
	end
	...
	if sideAvail ~= true then
		...
		net.log('[SLOTAUTH] 因sideAvail被拒 玩家 ' .. tostring(_playerInfo.name or playerID) .. ' 被拒绝切换（冷却中），信息: ' .. kickMsg)
		return false
	end
	...
	if balance ~= true then
		...
		local msg = '由于人数不平衡，你需要加入 ' .. sideName .. ' 以获得最好的游戏体验。若你正与朋友组队，可输入"-tb"来跳边（只能跳一次！）'
		net.send_chat_to(msg, playerID)
		net.log('[SLOTAUTH] 因balance被拒 玩家 ' .. tostring(_playerInfo.name or playerID) .. ' 被提示加入 ' .. sideName .. '（人数平衡限制）')
		return false
	end
