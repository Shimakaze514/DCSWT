...
function LoadLua.callbacks.onPlayerTryChangeSlot(playerID, side, slotID)
  ...
  local result=LoadLua.allowEnterSlot(playerID,_side,_slotID)
...
function LoadLua.allowEnterSlot (_playerID,_side,_slotID)
  ...
  if _unitRole ~= nil and _unitRole == "instructor" then --游戏管理员
      return LoadLua.findIDInTable(_playerID,_ucid,UserList.admin,'instructor')
  end
  if _unitRole ~= nil and _unitRole == "observer" then --观察员
    return LoadLua.findIDInTable(_playerID,_ucid,UserList.observer,'observer')
  end
  if _unitRole ~= nil and _unitRole == "artillery_commander" then --CA
    return LoadLua.findIDInTable(_playerID,_ucid,UserList.commander,'artillery_commander')
  end
...
function LoadLua.findIDInTable(_playerID,_ucid,table,commander)
  local allowed = false
